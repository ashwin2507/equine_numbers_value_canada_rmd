...
The R programming language and the following R packages were used to perform the analysis: knitr and tidyverse [@tidy_ref]. The code used to perform the analysis and create this report can be found here: https://github.com/ttimbers/equine_numbers_value_canada_rmd.
...
max_horses <- horse_pop %>% 
  ...
  summarize(Maximum = max(Value),
            Minimum = min(Value))
knitr::kable(max_horses, caption = "Table 1. Represents the Horse Population of Canada") |> 
  kableExtra::kable_styling(full_width = FALSE)
```

Below we zoom in and look at the province of Alberta:

```{r plot province, fig.width=5, fig.height=4, fig.cap= "Figure. Horse population in Alberta", fig.align='center'}
